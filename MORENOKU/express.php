<?php
	...
	class express
	{
		...
		function validarIR($c1,$identificacion){
			$inf=0;
			$sql="SELECT identificacion FROM ".$this->table." WHERE identificacion LIKE '".$identificacion."' ;";
			$res=mysqli_query($c1, $sql) or $_SESSION['Mysqli_Error'] = (mysqli_error($c1));
			if ($res) {
				if ($res->num_rows > 0) {
					$inf = 1;
		...
		function add($c1,$id_recolector,$fecha_orden){
			function validarAdd($id_recolector,$fecha_orden){
				$er=1;
				if(is_null($id_recolector)){ $er=0; }
				if(is_null($fecha_orden)){ $er=0; }
				return $er;
			}
			if (validarAdd($id_recolector,$fecha_orden)==1) {
				$sql="INSERT INTO ".$this->table." (`id_recolector`, `fecha_orden`) VALUES ('".$id_recolector."', '".$fecha_orden."');";
				$res=mysqli_query($c1,$sql) or $_SESSION['Mysqli_Error'] = (mysqli_error($c1));
				if ($res) {
					$inf="add";
				}else{
