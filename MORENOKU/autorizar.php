<?php
	...
	class autorizar
	{
		...
		function add($c1,$id_orden,$serie,$identificacion,$cable_hdmi,$cable_av,$fuente,$control_1,$estado,$horario_rec,$adicional,$id_recolector){
			function validarAdd($id_orden,$serie,$identificacion,$id_recolector){
				$er=1;
				if(is_null($id_orden)){ $er=0; }
				if(is_null($serie)){ $er=0; }
				if(is_null($identificacion)){ $er=0; }
				if(is_null($id_recolector)){ $er=0; }
				return $er;
			}
			if (validarAdd($id_orden,$serie,$identificacion,$id_recolector)==1) {
				$sql="INSERT INTO ".$this->table." (id_recolector_2, id_orden, serie, identificacion, cable_hdmi, cable_av, fuente,control_1,estado_rec,horario_rec, adicional) VALUES ('".$id_recolector."','".$id_orden."','".$serie."','".$identificacion."','".$cable_hdmi."','".$cable_av."','".$fuente."','".$control_1."','".$estado."','".$horario_rec."','".$adicional."');";
				$res=mysqli_query($c1,$sql) or $_SESSION['Mysqli_Error'] = (mysqli_error($c1));
				if ($res) {
					$inf="add";
				}else{
		...
		function edit($c1,$id,$serie,$cable_hdmi,$cable_av,$fuente,$control_1,$id_orden,$estado_rec,$horario_rec,$id_recolector_password,$id_recolector_2){
			function validarEdit($id,$id_orden,$serie,$id_recolector){
				$er=1;
				if(is_null($id)){ $er=0; }
				if(is_null($id_orden)){ $er=0; }
				if(is_null($serie)){ $er=0; }
			...
			if (validarEdit($id,$id_orden,$serie,$id_recolector)==1) {
				$sql="UPDATE ".$this->table." SET serie='".$serie."', id_orden='".$id_orden."', estado_rec='".$estado_rec."', cable_hdmi='".$cable_hdmi."', cable_av='".$cable_av."', fuente='".$fuente."', control_1='".$control_1."', horario_rec='".$horario_rec."', password_rec='".$id_recolector_password."', id_recolector_2='".$id_recolector_2."' WHERE id='".$id."' ;";
				$res=mysqli_query($c1,$sql) or $_SESSION['Mysqli_Error'] = (mysqli_error($c1));
				if ($res) {
					$inf="edit";
				}else{
